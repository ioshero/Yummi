 ...
 */
public class ServiceShowAdapter extends ArrayAdapter<Service> {


    MainActivity c;

    public ServiceShowAdapter(MainActivity context, ArrayList<Service> service) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (service.getAvailability() == 2) {
            ...
            if (service.getService().getId().isEmpty())
                tvName.setText(service.getService().getName() + "     " + String.valueOf(service.getPrice()) + " $");
