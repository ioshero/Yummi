...
public class ServiceAdapter extends ArrayAdapter<Service> {
    ...
    public ServiceAdapter(MainActivity context, ArrayList<Service> service) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (service.getAvailability() == 2) {
            ...
            if (service.getService().getId().isEmpty()) {
                tvName.setText(tvName.getText() + "     " + String.valueOf(service.getPrice()) + " $");
            }
...
//                            c.deleteEventService(service.getService().getId(), eventId, service.getUid());
                            for (EventPerformerModel prq : MainActivity.activeModel.getPerformers())
                                if (prq.getPerformerId().equalsIgnoreCase(service.getUid()))
